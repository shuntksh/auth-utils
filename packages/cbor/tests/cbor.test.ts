...
describe("CBOR", () => {
	...
	describe("Property-based tests", () => {
		...
		])("should handle any valid integer", (n) => {
			const encoded = CBOR.encode(n);
			const decoded = CBOR.decode(encoded);
			expect(decoded).toBe(n);
		});

		test.prop([fc.string()])("should handle any valid string", (s) => {
		...
		test.prop([fc.uint8Array({ minLength: 0, maxLength: 1000 })])(
			"should handle any valid byte array",
			(arr) => {
		...
		])("should handle any valid array", (arr) => {
		...
		])("should handle any valid object", (obj) => {
		...
		])("should handle edge cases for integers", (n) => {
			const encoded = CBOR.encode(n);
			const decoded = CBOR.decode(encoded);
			expect(decoded).toBe(n);
		});
		...
		])("should handle edge cases for strings", (s) => {
			const encoded = CBOR.encode(s);
			const decoded = CBOR.decode(encoded);
			expect(decoded).toBe(s);
		});
		...
		])("should handle edge cases for byte arrays", (arr) => {
			const buffer = arr.buffer;
			const encoded = CBOR.encode(buffer);
			const decoded = CBOR.decode(encoded);
			expect(new Uint8Array(decoded as ArrayBuffer)).toEqual(arr);
