...
if (import.meta.main) {
    ...
    Examples:
      node cli.mjs --uuid            Generate a single UUIDv7.
      node cli.mjs --snowflake -c 5  Generate 5 Snowflake IDs.
      node cli.mjs -w 1 -s -c 10     Generate 10 Snowflake IDs with worker ID 1.`;
	...
	while (args.length) {
		...
		switch (arg) {
			...
			case "--id":
			case "-w":
				flag.workerId = Number.parseInt(args.shift() || "0", 10);
				break;
			case "--count":
			case "-c":
				flag.count = Number.parseInt(args.shift() || "1", 10);
				break;
			case "--help":
			case "-h":
				console.log(usage);
	...
	if (flag.decode && flag.value) {
		try {
			if (UUIDv7.isValid(flag.value)) {
				const { timestamp, version, variant, randA, randB } = UUIDv7.parse(
					flag.value,
				);
				console.log(
					JSON.stringify({
